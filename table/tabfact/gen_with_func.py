...
import argparse
import nltk
from preprocess_example import string_in_table, stop_words2 as stop_words, date_match, isnum
...
# --trigger_word_dict=/home/ousuixin/Desktop/SASP/data/tabfact/trigger_word_all.json
...
parser.add_argument('--data_dict_file', help='tab file path')
parser.add_argument('--train_shard', default=90, help='whether to expand entities')
parser.add_argument('--trigger_word_dict', help='trigger word for function')
        ...
        for i, ent in enumerate(ents):
            if tk_idx <= ent['token_start']:
                ...
                if prev_ent['type'] == 'string_list' and \
                    not string_in_table(prev_ent['value'][0], tab, ignore_caption=True):
                ...
                if not(tk_idx >= ent['token_end']):
                    ...
                    if prev_ent['value'][0] == '' or not \
                        string_in_table(ent['value'][0], tab, ignore_caption=True):
                        continue
...
def extract_props_funcs(examples, tabs, combined_ents_l):
    ...
    for e, combined_ents in zip(examples, combined_ents_l):
        ...
        if e['id'] == "nt-30318": print(combined_ents)
